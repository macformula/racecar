 ...
 *module mounted on STM32469I-Discovery evaluation board.
 ******************************************************************************
 * @attention
 *
...
/** @defgroup STM32469I-Discovery_LCD_Private_TypesDefinitions STM32469I
 * Discovery LCD Private TypesDefinitions
 * @{
 */
...
/** @defgroup STM32469I-Discovery_LCD_Private_Defines STM32469I Discovery LCD
 * Private Defines
 * @{
 */
/**
...
/** @defgroup STM32469I-Discovery_LCD_Private_Macros STM32469I Discovery LCD
 * Private Macros
 * @{
 */
...
/** @defgroup STM32469I-Discovery_LCD_Exported_Variables STM32469I Discovery LCD
 * Exported Variables
 * @{
 */
...
/** @defgroup STM32469I-Discovery_LCD_Private_Variables STM32469I Discovery LCD
 * Private Variables
 * @{
 */
...
/** @defgroup STM32469I-Discovery_LCD_Private_FunctionPrototypes STM32469I
 * Discovery LCD Private FunctionPrototypes
 * @{
 */
...
/** @defgroup STM32469I-Discovery_LCD_Exported_Functions STM32469I Discovery LCD
 * Exported Functions
 * @{
 */
...
uint8_t BSP_LCD_InitEx(LCD_OrientationTypeDef orientation) {
    ...
    hdsivideo_handle.PacketSize =
        HACT; /* Value depending on display orientation choice
                 portrait/landscape */
    ...
    hdsivideo_handle.VerticalActive =
        VACT; /* Value depending on display orientation choice
                 portrait/landscape */
     ...
     * regions */
    /* Only useful when sending LP packets is allowed while streaming is active
     * in video mode */
    ...
    /* Specify for each region of the video frame, if the transmission of
     * command in LP mode is allowed in this region */
    ...
    /* LTDC clock frequency = PLLLCDCLK / LTDC_PLLSAI_DIVR_2 = 54.857 MHz / 2
     * = 27.429 MHz */
    ...
    if (Lcd_Driver_Type == LCD_CTRL_NT35510) {
        /***********************NT35510
         * Initialization********************************/
        /* Initialize the NT35510 LCD Display IC Driver (TechShine LCD IC
         * Driver) depending on configuration set in 'hdsivideo_handle'.
         */
 ...
 * time) and desactivating it later. This signal is only cabled on Discovery Rev
 * B and beyond.
 */
...
uint32_t BSP_LCD_ReadPixel(uint16_t Xpos, uint16_t Ypos) {
    ...
    if (hltdc_eval.LayerCfg[ActiveLayer].PixelFormat ==
        ...
        /* Read data value from SDRAM memory */
        ret =
            (*(__IO uint32_t*)(hltdc_eval.LayerCfg[ActiveLayer].FBStartAdress +
                               (4 * (Ypos * BSP_LCD_GetXSize() + Xpos))) &
             0x00FFFFFF);
    } else if ((hltdc_eval.LayerCfg[ActiveLayer].PixelFormat ==
                LTDC_PIXEL_FORMAT_RGB565) ||
               (hltdc_eval.LayerCfg[ActiveLayer].PixelFormat ==
                LTDC_PIXEL_FORMAT_ARGB4444) ||
               (hltdc_eval.LayerCfg[ActiveLayer].PixelFormat ==
                LTDC_PIXEL_FORMAT_AL88)) {
        /* Read data value from SDRAM memory */
        ret = *(__IO uint16_t*)(hltdc_eval.LayerCfg[ActiveLayer].FBStartAdress +
                                (2 * (Ypos * BSP_LCD_GetXSize() + Xpos)));
    } else {
        /* Read data value from SDRAM memory */
        ret = *(__IO uint8_t*)(hltdc_eval.LayerCfg[ActiveLayer].FBStartAdress +
                               (2 * (Ypos * BSP_LCD_GetXSize() + Xpos)));
    }
...
void BSP_LCD_Clear(uint32_t Color) {
    ...
    LL_FillBuffer(ActiveLayer,
                  (uint32_t*)(hltdc_eval.LayerCfg[ActiveLayer].FBStartAdress),
...
void BSP_LCD_DisplayStringAt(uint16_t Xpos, uint16_t Ypos, uint8_t* Text,
    ...
    switch (Mode) {
        ...
        case RIGHT_MODE: {
            refcolumn =
                -Xpos + ((xsize - size) * DrawProp[ActiveLayer].pFont->Width);
            break;
        }
        default: {
            refcolumn = Xpos;
...
void BSP_LCD_DrawHLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length) {
    ...
    Xaddress = (hltdc_eval.LayerCfg[ActiveLayer].FBStartAdress) +
...
void BSP_LCD_DisplayOn(void) {
    ...
    HAL_DSI_ShortWrite(&(hdsi_eval), hdsivideo_handle.VirtualChannelID,
...
void DSI_IO_WriteCmd(uint32_t NbrParams, uint8_t* pParams) {
    if (NbrParams <= 1) {
        HAL_DSI_ShortWrite(&hdsi_eval, LCD_Driver_ID,
                           DSI_DCS_SHORT_PKT_WRITE_P1, pParams[0], pParams[1]);
    } else {
        HAL_DSI_LongWrite(&hdsi_eval, LCD_Driver_ID, DSI_DCS_LONG_PKT_WRITE,
...
int32_t DSI_IO_ReadCmd(uint32_t Reg, uint8_t* pData, uint32_t Size) {
    ...
    if (HAL_DSI_Read(&hdsi_eval, LCD_Driver_ID, pData, Size,
...
__weak void BSP_LCD_MspDeInit(void) {
    ...
    /** @brief Force and let in reset state LTDC, DMA2D and DSI Host + Wrapper
     * IPs */
...
static void LL_ConvertLineToARGB8888(void* pSrc, void* pDst, uint32_t xSize,
    ...
    if (HAL_DMA2D_Init(&hdma2d_eval) == HAL_OK) {
        if (HAL_DMA2D_ConfigLayer(&hdma2d_eval, 1) == HAL_OK) {
            if (HAL_DMA2D_Start(&hdma2d_eval, (uint32_t)pSrc, (uint32_t)pDst,
                                xSize, 1) == HAL_OK) {
                /* Polling For DMA transfer */
                HAL_DMA2D_PollForTransfer(&hdma2d_eval, 10);
            }
