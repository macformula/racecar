...
TEST(DriverInterface, TestSequence) {
    DiFsm fsm{};
    DiFsm::Output out;
    ...
    {  // stay in INIT until governor commands INIT
        ...
        EXPECT_EQ(out.status, DiSts::INIT);
        ...
        out = fsm.Update(in, time_ms++);
        EXPECT_EQ(out.status, DiSts::WAITING_FOR_DRVR);
        EXPECT_FALSE(out.ready_to_drive);
        EXPECT_FALSE(out.speaker_enable);
    }
...
TEST(DriverInterface, HvStartReq) {
    ...
    {  // Start the motor only if the high-voltage is on and the driver presses
       // the button
        ...
        out = fsm.Update(in, time_ms++);
        EXPECT_EQ(out.status, DiSts::HV_START_REQ);
        EXPECT_FALSE(out.ready_to_drive);
        EXPECT_FALSE(out.speaker_enable);
        ...
        out = fsm.Update(in, time_ms++);
        EXPECT_EQ(out.status, DiSts::MOTOR_START_REQ);
        EXPECT_FALSE(out.ready_to_drive);
        EXPECT_FALSE(out.speaker_enable);
    }
}
TEST(DriverInterface, MotorStartReq) {
    {  // Only enter RUNNING when both command and pedal are valid
        ...
        in.brake_pedal_pos = 0.5;
        out = fsm.Update(in, time_ms++);
        EXPECT_EQ(out.status, DiSts::RUNNING);
        EXPECT_TRUE(out.ready_to_drive);
        EXPECT_TRUE(out.speaker_enable);
    }
}
TEST(DriverInterface, test_speaker) {
...
TEST(DriverInterface, test_shutdown) {
    {  // shutdown
